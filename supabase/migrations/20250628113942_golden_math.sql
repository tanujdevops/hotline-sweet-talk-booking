...
CREATE OR REPLACE FUNCTION test_agent_availability_view()
RETURNS TABLE(
  plan_type TEXT,
  status TEXT,
  agent_count INTEGER,
...
SELECT * FROM test_agent_availability_view();
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION test_agent_availability_view() TO service_role;
GRANT EXECUTE ON FUNCTION test_agent_availability_view() TO postgres;

-- Add comment
COMMENT ON FUNCTION test_agent_availability_view() IS 'Safely tests agent availability without causing errors';