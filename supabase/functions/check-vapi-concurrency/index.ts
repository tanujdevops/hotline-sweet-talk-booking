...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: account, error: accountError } = await supabaseClient.from('vapi_accounts').select('id, current_active_calls, max_concurrent_calls').eq('is_active', true).limit(1).single();
    if (accountError) {
    ...
    if (!canMakeCall) {
      const { count, error: queueError } = await supabaseClient.from('call_queue').select('*', {
        count: 'exact',
        head: true
      }).eq('status', 'queued');
      if (queueError) {
