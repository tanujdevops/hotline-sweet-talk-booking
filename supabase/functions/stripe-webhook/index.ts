  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        const { data: bookingWithUser, error: userFetchError } = await supabaseClient
          .from('bookings')
          .select('id, users(name, phone)')
          .eq('id', bookingId)
          .single();
        if (userFetchError || !bookingWithUser || !bookingWithUser.users) {
          console.error(`Error fetching user info for booking ${bookingId}:`, userFetchError);
        } else {
          try {
            const { error: processError } = await supabaseClient.functions.invoke('initiate-vapi-call', {
              body: {
